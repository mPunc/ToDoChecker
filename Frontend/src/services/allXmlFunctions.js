export async function generateXml() {
    return(
        await fetch("http://localhost:5001/todo/generate-xml", {method: "GET"})
...
export async function getAllConsole() {
    return(
        await fetch("http://localhost:5001/todo/get-all", {method: "GET"})
            .then(response => response.text())
            .then(str => {
...
export async function createNewTask() {
    ...
    //xml.appendChild(Object.assign(xmlDoc.createElement("Id"), { textContent: "4" }));
    ...
    return(
        await fetch("http://localhost:5001/todo", {
            method: "POST",
            headers: {
...
export async function deleteTask(id) {
  return fetch(`http://localhost:5001/todo/${id}`, {
    method: "DELETE",
    headers: {
      "Content-Type": "application/xml" // or "application/json", depending on your backend
    }
  })
  .then(response => {
